      ...
      <div class="row">
       ...
       <div class="portlet light">
              <div class="portlet-title">
                <div class="caption">
                  <span class="caption-subject uppercase">CASHIER VIOLATION FORWARDED</span>
                  <!-- &nbsp;|&nbsp;&nbsp;<button type="button" class="btn green btn-sm" id="submit_violation_btn">SUBMIT VIOLATION</button> -->
                </div>     
              </div>
            <div class="portlet-body">
                <form id="denomination_form">
                    <table class="table table-bordered table-hover table-condensed display" id="violation_table_forwarded">
                        <thead>
                            <tr>                                                                                                             
                              <th>
...
<div class="modal fade" id="cancel_violation_modal" tabindex="-1" role="basic" aria-hidden="true" style="display: none;">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button type="button" class="btn default btn-sm" data-dismiss="modal"><span class="md-click-circle md-click-animate" style="height: 65px; width: 65px; top: -14.5px; left: 1.51563px;"></span>Close</button>
                <button type="button" class="btn btn-success btn-sm" id="cancel_violation_btn">Continue</button>
            </div>
        </div>
    <!-- /.modal-content -->
    </div>
...
<script type="text/javascript">
    ...
    $("#cs_violation_for").addClass("active"); 
    var violation_table_forwarded;

    violation_table_forwarded = $('#violation_table_forwarded').DataTable(
    {
        'ajax':'<?php echo site_url('main_controller/violation_table_forwarded_con');?>',  
        'columnDefs': [
    ...
    function cancel_violation(id)
    {
      $('#cancel_violation_modal').modal('show'); 
      
      $('textarea#reason').val("");

      $("#cancel_violation_btn").unbind('click').bind('click', function()       
      { 
        $('#cancel_violation_btn').prop('disabled', true);

        var reason = $('textarea#reason').val(); 

        if(reason == "")
        {
          alert('Please justify reason!');
        }
        else
        {
          $.ajax(
          {
              url:'<?php echo site_url('main_controller/cancel_violation_con');?>',
              type:'POST',
              ...
              {   
                  ...
                  // $('#submit_violation_modal').modal('hide'); 
                  // $('#success_modal').modal('show');
                  // $('#submit_violation_modal_btn').prop('disabled', false);
                  // violation_table.ajax.reload(null, false);  
